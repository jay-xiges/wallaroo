    ...
    try
      ...
      | let m: cwm.NotifyMsg =>
        ...
        try
          ...
          // Either this stream is already active locally, in which case
          // respond with notify_ack(success = false)
          // or it's not on this instance, in which case refer to the listener's
          // registry
          // Listener will either know of another listener on the same worker,
          // in which case the reply will be notify_ack(success = false)
          // or it will escalate to the leader lister, which will do the same
          // check...
          // If no one has claimed this stream, we may respond with
          // notify_ack(success = true)

          // stream is not in local map
          if not _stream_map.contains(m.stream_id) then
            // Adding new stream to worker stream map
            // Go through the _stream_registry
            (_stream_registry as ConnectorSourceListener[In])
              .stream_notify(_session_tag, m.stream_id, m.stream_name,
                m.point_of_ref, _connector_source as ConnectorSource[In])

            // TODO [source-migration]: Nisan: remove this?
            // Scott says this is to prevent double notifies from triggering
            // Nisan: probably a good idea to split this to _pending_notify
            // so that we can proactively deactivate during rollback to make
            // sure we don't leave any streams in a state of limbo
            _stream_map(m.stream_id) = _StreamState(true, 0, 0, 0, 0)
          else
            // Stream already in worker map, respond with negative notify_ack
          ...
          try
            ...
            try
              ...
              if cwm.Eos.is_set(m.flags) then
                ...
                (_stream_registry as ConnectorSourceListener[In]).stream_update(
  ...
  fun ref rollback(checkpoint_id: CheckpointId, payload: ByteSeq val) =>
    ...
    try
      while true do
        ...
        (_stream_registry as ConnectorSourceListener[In]).stream_update(stream_id, barrier_checkpoint_id,
