...
class ConnectorSink2Notify
  ...
  var _point_of_ref: cp.MessageId = 1
  ...
  fun ref connected(conn: WallarooOutgoingNetworkActor ref) =>
    @printf[I32]("ConnectorSink2 connected\n".cstring())
    conn.expect(4)
  ...
  fun ref closed(conn: WallarooOutgoingNetworkActor ref) =>
    @printf[I32]("ConnectorSink2 connection closed\n".cstring())

  fun ref dispose() =>
    @printf[I32]("ConnectorSink2 connection dispose\n".cstring())

  fun ref connect_failed(conn: WallarooOutgoingNetworkActor ref) =>
    @printf[I32]("ConnectorSink2 connection failed\n".cstring())
  ...
  =>
    if _header then
      try
        ...
        @printf[I32]("QQQ: ConnectorSink2 got header\n".cstring())
      ...
      @printf[I32]("QQQ: ConnectorSink2 got body: %s\n".cstring(), _print_array[U8](data').cstring())
      try
  ...
  fun _send_msg(conn: WallarooOutgoingNetworkActor ref, msg: cp.Message) =>
    ...
    @printf[I32]("Sink2 b1: size %d\n".cstring(), b.size())
    w2.write(b)

    let b2 = recover trn w2.done() end
    try (conn as ConnectorSink2 ref)._writev(consume b2, None) else Fail() end
